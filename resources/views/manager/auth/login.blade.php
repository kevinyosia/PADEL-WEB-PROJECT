...
<body class="bg-[#44664d] h-screen flex items-center justify-center font-sans text-gray-800 relative">
    ...
    <div class="w-full max-w-5xl flex flex-col md:flex-row items-center justify-between px-10">
        ...
        <div class="w-full md:w-1/2 flex justify-center md:justify-start mb-10 md:mb-0">
            <img src="{{ asset('images/logo padel bandeja.png') }}" alt="Bandeja Padel Arena Logo" class="w-3/4 max-w-md object-contain">
        </div>
            ...
            <div class="bg-[#EFEBE0] p-10 rounded-3xl shadow-sm w-full max-w-sm border border-gray-200">
                ...
                <form method="POST" action="{{ route('manager.login.submit') }}">
                    ...
                    <div class="mb-6">
                        ...
                        <input id="email" type="email" name="email" value="{{ old('email') }}" required autofocus class="w-full px-4 py-2 rounded-full border border-[#A7A7A7] bg-transparent focus:ring-[#44664d] focus:border-[#44664d]">
                        <x-input-error :messages="$errors->get('email')" class="mt-2 text-red-500 text-sm" />
                    </div>

                    <div class="mb-8">
                        <label for="password" class="block text-sm font-semibold mb-2">Password</label>
                        <input id="password" type="password" name="password" required class="w-full px-4 py-2 rounded-full border border-[#A7A7A7] bg-transparent focus:ring-[#44664d] focus:border-[#44664d]">
                        <x-input-error :messages="$errors->get('password')" class="mt-2 text-red-500 text-sm" />
                    </div>

                    <div class="flex justify-center">
                        <button type="submit" class="bg-[#44664d] text-white px-10 py-3 rounded-full font-bold hover:bg-opacity-90 transition w-3/4 shadow-md">
                            Masuk
                        </button>
                    </div>
                </form>
