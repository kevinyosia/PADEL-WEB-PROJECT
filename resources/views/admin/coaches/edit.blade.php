        ...
        <div class="form-card">
            <form method="POST" action="{{ route('admin.coaches.update', $coach) }}" enctype="multipart/form-data">
                ...
                <div class="form-section">
                    ...
                    <div style="display:grid;grid-template-columns:1fr;gap:10px;">
                        @foreach(['mon'=>'Senin','tue'=>'Selasa','wed'=>'Rabu','thu'=>'Kamis','fri'=>'Jumat'] as $key => $label)
                            @php
                                ...
                                $coachDay = $coach->schedule[$key] ?? [];
                                $isActive = is_array($oldDay)
                                    ? (bool) ($oldDay['active'] ?? false)
                                    : (is_array($coachDay) ? (bool) ($coachDay['active'] ?? false) : false);
                                $sessions = is_array($oldDay)
                                    ? ($oldDay['sessions'] ?? [])
                                    : (is_array($coachDay) ? ($coachDay['sessions'] ?? []) : []);
                    ...
                    <p class="field-hint" style="margin-top:10px;">Admin dapat menambah atau menghapus session kapan saja jika coach berhalangan.</p>
                </div>
